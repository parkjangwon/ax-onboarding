...
온보딩을 마치면 사용자 홈 디렉토리(`~/.ax/runbooks/`)에 바로 쓸 수 있는 마크다운 런북이 생성됩니다. 프롬프트를 어떻게 써야 할지 고민할 필요 없이 런북을 호출하면 됩니다.

1. `01-troubleshoot.md`: SSH 로그 조회부터 코드 라인 특정, 방어 코드 패치 제안까지 이어지는 표준 장애 분석 런북
